...
namespace AKStreamWeb.Services
{
  public static class SipServerService
  {
    ...
    /// <param name="rs"></param>
    /// <returns></returns>
    public static bool GetSipDeviceStatus(string deviceId, string channelId, out ResponseStruct rs)
    {
      ...
      {
        GCommon.Logger.Warn($"[{Common.LoggerHead}]->获取sip设备状态失败->{deviceId}-{channelId}->{JsonHelper.ToJson(rs)}");
      ...
      try
      {
        ...
        if (sipMethodProxy.GetSipDeviceStatus(sipDevice, out rs))
        {
          GCommon.Logger.Debug(
$"[{Common.LoggerHead}]->获取设备状态信息成功->{sipDevice.RemoteEndPoint.Address.MapToIPv4().ToString()}-{sipDevice.DeviceId}\r\n{JsonHelper.ToJson(sipDevice.DeviceStatus)}");
          return true;
        }

        GCommon.Logger.Warn($"[{Common.LoggerHead}]->获取sip设备状态信息失败->{deviceId}-{channelId}->{JsonHelper.ToJson(rs)}");
        return false;
      }
      catch (Exception e)
      {

        GCommon.Logger.Warn($"[{Common.LoggerHead}]->获取sip设备状态失败->{deviceId}-{channelId}->{e.Message}");
    ...
    public static bool StopLiveVideo(int taskId, string ssrcId, out ResponseStruct rs)
    {
      ...
      // }
      GCommon.Logger.Info(
          $"[{Common.LoggerHead}]->VideoOnlineInfo.DeleteMany->StopLiveVideo->{record.SipDevice.DeviceId}-{record.SipChannel.DeviceId}-{record.Stream}");
        ...
        ushort? rtpPort = 0)
    {
      ...
      if (!isTimeout)
      {
        ...
        // }
        GCommon.Logger.Info(
        $"[{Common.LoggerHead}]->VideoOnlineInfo.DeleteMany->LiveVideo->请求Sip推回放流失败->{record.SipDevice.DeviceId}-{record.SipChannel.DeviceId}-{record.Stream}");
      ...
      GCommon.Logger.Info(
          $"[{Common.LoggerHead}]->VideoOnlineInfo.DeleteMany->LiveVideo->请求Sip推回放流成功->{record.SipDevice.DeviceId}-{record.SipChannel.DeviceId}-{record.Stream}");
        ...
        ushort? rtpPort = 0)
    {
      ...
      if (rtpPort == null || rtpPort == 0) //如果没指定rtp端口，就申请一个
      {
        if (videoChannel.DefaultRtpPort == false) //非使用固定端口，则申请
        {
          try
          {
            ...
            {
              GCommon.Logger.Warn(
                 $"[{Common.LoggerHead}]->请求Sip推流失败->申请RTP端口失败->{deviceId}-{channelId}->{JsonHelper.ToJson(rs)}");
          ...
          catch (Exception ex)
          {
            ...
            GCommon.Logger.Warn(
               $"[{Common.LoggerHead}]->请求Sip推流失败->申请RTP端口失败2->{deviceId}-{channelId}->{JsonHelper.ToJson(rs)}");
      ...
      if (!openRtpPort.Stream.Trim().Equals(sipChannel.Stream))
      {
        ...
        GCommon.Logger.Warn($"[{Common.LoggerHead}]->请求Sip推流失败->申请RTP端口失败3->{deviceId}-{channelId}->{JsonHelper.ToJson(rs)}");
      ...
      try
      {
        ...
        if (!rs.Code.Equals(ErrorNumber.None) || liveVideoRet == false)
        {
          ...
          GCommon.Logger.Warn($"[{Common.LoggerHead}]->请求Sip推流失败->申请RTP端口失败4->{deviceId}-{channelId}->{JsonHelper.ToJson(rs)}");
      ...
      if (!isTimeout)
      {
        ...
        GCommon.Logger.Warn($"[{Common.LoggerHead}]->请求Sip推流失败->申请RTP端口失败5->{deviceId}-{channelId}->{JsonHelper.ToJson(rs)}");
        ...
        //             }
        GCommon.Logger.Info(
        $"[{Common.LoggerHead}]->VideoOnlineInfo.DeleteMany->LiveVideo->请求Sip推流失败");
...
x.MainId.Equals(videoChannel.MainId) && x.MediaServerId.Equals(videoChannel.MediaServerId));
      GCommon.MongoDb.VideoOnlineInfo.DeleteMany(delLanbda2);
      GCommon.Logger.Info(
          $"[{Common.LoggerHead}]->VideoOnlineInfo.DeleteMany->LiveVideo->请求Sip推流成功");
      //GCommon.Ldb.VideoOnlineInfo.Insert(videoChannelMediaInfo);
      GCommon.MongoDb.VideoOnlineInfo.InsertOne(videoChannelMediaInfo);
      //         lock (GCommon.MongoDb.MongoDBLockObj)
      //         {
    ...
    public static bool IsLiveVideo(string deviceId, string channelId, out ResponseStruct rs)
    {
      ...
      // lock(GCommon.MongoDb.MongoDBLockObj)
      // {
      //     obj = GCommon.MongoDb.VideoOnlineInfo.Find(lanbda).FirstOrDefault();
      // }
    ...
    //   {
    //     Expression<Func<VideoChannelMediaInfo, bool>> lanbda = (x => x.MediaServerStreamInfo.Ssrc == ssrcId);
    //     vBack = GCommon.MongoDb.VideoOnlineInfo.Find(lanbda).FirstOrDefault();
    //   }
      ResMediaServerOpenRtpPort openRtpPort;
      openRtpPort = new ResMediaServerOpenRtpPort()
      {
